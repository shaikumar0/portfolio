// teleport.js
// Main interactive background script for the portfolio
// GSAP is included in HTML but the following script intentionally does NOT use GSAP
// for overlay fade, parallax smoothing or star twinkle (those features removed per request).
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // defensive trim for stored theme
  let currentTheme = (localStorage.getItem('siteTheme') || 'land').trim();
  let currentMode  = (localStorage.getItem('siteMode')  || 'dark').trim();
  let lowPower     = JSON.parse(localStorage.getItem('siteLowPower') || 'false'); // kept but not surfaced in UI

  const clamp = (v,a,b)=>Math.max(a,Math.min(b,v));
  const rnd = (a,b)=>Math.random()*(b-a)+a;

  // --- color helpers (returns "r,g,b" for use inside rgba(...,ALPHA) )
  ...
  function colorToRGBList(colorStr){
    ...
    // handle rgb(a) strings like "rgb(56,243,255)" or "rgba(56,243,255,0.9)"
    const m = colorStr.match(/rgba?\s*\(\s*(\d{1,3})\s*,\s*(\d{1,3})\s*,\s*(\d{1,3})/i);
  ...
  */
  function applyThemeMode() {
    ...
    // overlay fade removed: set instantly to hidden
    ...
    // ensure front-layer static in space theme visually:
    if(currentTheme === 'space'){
      layerFront.style.transform = 'translate3d(0px, 0px, 0) scale(1)';
    }
  }
  ...
  themeButtons.forEach(btn => {
    btn.addEventListener('click', ()=> {
      ...
      currentTheme = (btn.dataset.theme || '').trim();
      if(!currentTheme) currentTheme = 'land';
      applyThemeMode();
    });
  });
  ...
  class Jellyfish {
    ...
    reset(init=false){
      ...
      // choose accent color from CSS vars: left (--accent1) for dark, right (--accent2) for light
      const rootStyles = getComputedStyle(document.documentElement);
      const accentLeft  = rootStyles.getPropertyValue('--accent1') || '#38f3ff';
      const accentRight = rootStyles.getPropertyValue('--accent2') || '#8b5cf6';
      const chosen = (mode === 'light' ? accentRight : accentLeft).trim();
      const rgbList = colorToRGBList(chosen);
      // colorMain is used like `${this.colorMain}0.95)` in draw(), so keep trailing comma form that draw expects
      this.colorMain = `rgba(${rgbList},`;
  ...
  class Star {
    ...
    // star twinkle removed (function intentionally left empty)
    twinkleNow(){
      // intentionally disabled per request (no twinkle)
    }
    update(dt){
  ...
  window.addEventListener('mousemove', (e)=>{ mouse.x = e.clientX; mouse.y = e.clientY; updateParallaxTransforms(); });
  window.addEventListener('mouseleave', ()=>{ mouse.x = -9999; mouse.y = -9999; updateParallaxTransforms(); });
  window.addEventListener('mousedown', ()=>{ mouse.down = true; });
  window.addEventListener('mouseup', ()=>{ mouse.down = false; });

  /* Click handler: star twinkle removed (no GSAP) */
  canvas.addEventListener('click', (e)=>{
    // click interaction intentionally no-op for twinkle per request.
    // we still keep click handler in case you want other interactions later.
  });
  ...
  */
    ...
    setMode: (md) => {
      ...
      for(const o of BGobjects){
        ...
        if(o instanceof Jellyfish){
          // recolor jellyfish to the appropriate accent color (left for dark, right for light)
          const rootStyles = getComputedStyle(document.documentElement);
          const accentLeft  = rootStyles.getPropertyValue('--accent1') || '#38f3ff';
          const accentRight = rootStyles.getPropertyValue('--accent2') || '#8b5cf6';
          const chosen = (mode === 'light' ? accentRight : accentLeft).trim();
          o.colorMain = `rgba(${ colorToRGBList(chosen) },`;
        }
  ...
  /* ===========================
     ...
     - smoothing removed: transforms are applied immediately (no GSAP smoothing/ticker)
     - front layer is static in 'space' theme (no hover/scroll movement)
     =========================== */
  let mouseX = 0, mouseY = 0;
  window.addEventListener('mousemove', (e)=>{ mouseX = (e.clientX / window.innerWidth - 0.5); mouseY = (e.clientY / window.innerHeight - 0.5); /* transforms updated in handler above */ });

  window.addEventListener('scroll', ()=>{ updateParallaxTransforms(); });
  ...
  function updateParallaxTransforms(){
    ...
    if(theme === 'space'){
      ...
      // freeze front (neptune) so it doesn't move with hover or scroll
      layerFront.style.transform = `translate3d(0px, 0px, 0) scale(1)`;
    } else {
      layerBack.style.transform = `translate3d(${xBack}px, ${backY + mouseY * 10}px, 0) scale(1.02)`;
      layerMid.style.transform  = `translate3d(${xMid}px, ${backY*1.5 + mouseY * 18}px, 0) scale(1.01)`;
  ...
  /* ========================
     ...
     ======================== */
  applyThemeMode();
  // ensure parallax initial transform set
  updateParallaxTransforms();
});
